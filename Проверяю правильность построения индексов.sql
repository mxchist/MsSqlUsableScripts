declare @index_name nvarchar(255), @index_id int;
...
where  ic.is_included_column = 0
	and si.object_id = ( select OBJECT_ID('dbo.ORDERS', 'U') )
	and si.type <> 1
;
...
set @statement += 'from dbo.ORDERS' + @enter;
--print @statement;
...
begin
	...
	declare statement_cursor cursor fast_forward local for
	select
		sc.name
	from sys.index_columns as ic
	inner join sys.indexes as si on si.index_id = ic.index_id and si.object_id = ic.object_id
	...
	fetch next from statement_cursor into @column_name;
	if @@FETCH_STATUS = 0
		set @statement = 'select' + @enter + ' count(distinct ' + @column_name + ')' + @enter;

	fetch next from statement_cursor into @column_name;
	while @@FETCH_STATUS = 0
	begin
		set @statement += ', count(distinct ' + @column_name + ')' + @enter;
		fetch next from statement_cursor into @column_name;
	end
